...
local function rebuild()
    ...
    state.lot_ids = {};
    state.drop_ids = {};
    for _, id in ipairs(state.settings.pass or T{}) do
        id = tonumber(id);
        if id ~= nil then state.pass_ids[id] = true; end
    end
    for _, id in ipairs(state.settings.lot or T{}) do
        id = tonumber(id);
        if id ~= nil then state.lot_ids[id] = true; end
    end
    for _, id in ipairs(state.settings.drop or T{}) do
        id = tonumber(id);
        if id ~= nil then state.drop_ids[id] = true; end
    end
end
...
local function find_ids(query)
    ...
    if query == 'pool' then
        ...
        if inventory ~= nil then
            for slot = 0, 9 do
                ...
                if entry ~= nil and entry.ItemId ~= nil and entry.ItemId > 0 and entry.ItemId ~= 65535 then
    ...
    if groups[query] ~= nil then
        local result = {};
        for _, id in ipairs(groups[query]) do result[id] = true; end
        return result;
    end
    ...
    for id = 1, 65535 do
        ...
        if item ~= nil and item.Name ~= nil then
            ...
            if name:match(pattern) ~= nil or log_name:match(pattern) ~= nil then result[id] = true; end
        end
    end
    return result;
end
...
local function list_contains(list, id)
    for _, current in ipairs(list) do
        if tonumber(current) == id then return true; end
    end
    return false;
end
    ...
    for id, _ in pairs(ids) do
        ...
        elseif operation == 'remove' then
            for index = #list, 1, -1 do
                if tonumber(list[index]) == id then table.remove(list, index); count = count + 1; end
            end
        end
    end
    if count == 0 then print_error('No matching items were changed.'); return; end
    save();
    if kind == 'drop' then scan_inventory(); else scan_pool(); end
    print_message(('%s %u item(s) %s the %s list.'):format(operation == 'add' and 'Added' or 'Removed', count, operation == 'add' and 'to' or 'from', kind));
end
...
scan_inventory = function()
    ...
    for slot = 1, maximum do
        ...
        if id > 0 and id ~= 65535 and state.drop_ids[id] then
    ...
    for key, _ in pairs(state.drop_handled) do
        if not active[key] then state.drop_handled[key] = nil; end
    end
end
...
local function queue_action(slot, item_id)
    if slot == nil or slot < 0 or slot > 9 or item_id == nil or item_id <= 0 or item_id == 65535 then return; end
...
scan_pool = function()
    ...
    for key, _ in pairs(state.handled) do
        if not active[key] then state.handled[key] = nil; end
    end
end
...
local function print_list(kind)
    ...
    for _, id in ipairs(list) do print_message(('  %s [%u]'):format(item_name(tonumber(id)), tonumber(id))); end
end
...
ashita.events.register('packet_in', 'packet_in_cb', function(e)
    ...
    elseif e.id == 0x00D3 then
        ...
        if slot ~= nil and slot < 10 then
            ...
            for key, _ in pairs(state.handled) do
                if key:sub(1, #prefix) == prefix then state.handled[key] = nil; end
            end
        end
    ...
    for index = #state.drop_queue, 1, -1 do
        ...
        if now >= pending.due then
            ...
            if current ~= nil and tonumber(current.Id) == pending.item_id and state.drop_ids[pending.item_id] then
                ...
                if state.settings.verbose then print_message(('Dropping %s x%u.'):format(item_name(pending.item_id), count)); end
            end
            break;
        end
    end
    ...
    for index = #state.queue, 1, -1 do
        ...
        if now >= pending.due then
            ...
            if current ~= nil and current.ItemId == pending.item_id then
                ...
                local configured = state.lot_ids[pending.item_id] and 'lot' or (state.pass_ids[pending.item_id] and 'pass' or nil);
                if configured == pending.action and ((pending.action == 'pass' and lot ~= 65535) or (pending.action == 'lot' and (lot == 0 or lot >= 65535))) then
                    send_action(pending.action, pending.slot);
                    if state.settings.verbose then print_message(('%s %s.'):format(pending.action == 'lot' and 'Lotting' or 'Passing', item_name(pending.item_id))); end
                end
            end
        end
    end
...
ashita.events.register('command', 'command_cb', function(e)
    ...
    if #args == 0 or not args[1]:any('/treasury', '/tr') then return; end
    e.blocked = true;
    ...
    if command:any('pass', 'p', 'lot', 'l', 'drop', 'd') then
        ...
        elseif operation == 'list' then print_list(kind);
        elseif operation == 'clear' then state.settings[kind] = T{}; save(); print_message(kind .. ' list cleared.');
        else print_help(); end
        ...
        if inventory ~= nil then
            for slot = 0, 9 do
                ...
                if entry ~= nil and entry.ItemId ~= nil and entry.ItemId > 0 and entry.ItemId ~= 65535 then
                    ...
                    if command == 'passall' and lot ~= 65535 then send_action('pass', slot);
                    elseif command == 'lotall' and (lot == 0 or lot >= 65535) then send_action('lot', slot);
                    elseif command == 'done' and lot == 0 then send_action('pass', slot); end
                end
            end
        end
    elseif command == 'delay' and tonumber(args[3]) ~= nil then
        state.settings.delay = math.max(0, tonumber(args[3])); save(); print_message(('Delay set to %.2f seconds.'):format(state.settings.delay));
    elseif command == 'verbose' then
        ...
        save(); print_message('Verbose output ' .. (state.settings.verbose and 'enabled.' or 'disabled.'));
    elseif command:any('reload', 'rl') then settings.reload(); scan_pool(); scan_inventory(); print_message('Settings reloaded.');
    elseif command:any('help', 'h') then print_help();
    else print_help(); end
end);
